...
Factor that should multiply the lapse. This has the effect of rescaling the time
coordinate so that $t' = \code{lapsefactor}~ t$. Note that this also has the
effect of rescaling the shift by a factor $1/\code{lapsefactor}$ (see xform.nb
for more details).
