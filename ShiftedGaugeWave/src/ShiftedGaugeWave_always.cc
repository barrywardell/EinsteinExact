...
static void ShiftedGaugeWave_always_Body(cGH const * restrict const cctkGH, int const dir, int const face, CCTK_REAL const normal[3], CCTK_REAL const tangentA[3], CCTK_REAL const tangentB[3], int const min[3], int const max[3], int const n_subblock_gfs, CCTK_REAL * restrict const subblock_gfs[])
{
  ...
  for (k = min[2]; k < max[2]; k++)
  {
    for (j = min[1]; j < max[1]; j++)
    {
      for (i = min[0]; i < max[0]; i++)
      {
        ...
        CCTK_REAL betap1 = -(INV(1 + Sin(2*Pi*(X - 
          cctk_time)*pow(ToReal(period),-1))*ToReal(amp))*Sin(2*Pi*(X - 
          cctk_time)*INV(ToReal(period)))*ToReal(amp));
        ...
        CCTK_REAL alpL = INV(sqrt(1 + Sin(2*Pi*(X - 
          cctk_time)*INV(ToReal(period)))*ToReal(amp)));
