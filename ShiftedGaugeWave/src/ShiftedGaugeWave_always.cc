...
static void ShiftedGaugeWave_always_Body(cGH const * restrict const cctkGH, int const dir, int const face, CCTK_REAL const normal[3], CCTK_REAL const tangentA[3], CCTK_REAL const tangentB[3], int const imin[3], int const imax[3], int const n_subblock_gfs, CCTK_REAL * restrict const subblock_gfs[])
{
    ...
    CCTK_REAL_VEC_SIZE)
  {
    ...
    CCTK_REAL_VEC K11 = 
      kneg(kdiv(kmul(kcos(kmul(csetemp6,kmul(csetemp7,ToReal(2*Pi)))),ToReal(amp*Pi)),kmul(kmadd(ksin(kmul(csetemp6,kmul(csetemp7,ToReal(2*Pi)))),ToReal(amp*period),ToReal(period)),alpL)));
