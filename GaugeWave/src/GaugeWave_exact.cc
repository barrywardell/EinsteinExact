...
extern "C" void GaugeWave_exact(CCTK_ARGUMENTS)
{
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering GaugeWave_exact_Body");
  }
  if (cctk_iteration % GaugeWave_exact_calc_every != GaugeWave_exact_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "GaugeWave::dtlapse_exact",
    "GaugeWave::dtshift_exact",
    "grid::coordinates",
    "GaugeWave::lapse_exact",
    "GaugeWave::metric_exact",
    "GaugeWave::shift_exact"};
  AssertGroupStorage(cctkGH, "GaugeWave_exact", 8, groups);
  
  
  LoopOverEverything(cctkGH, GaugeWave_exact_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving GaugeWave_exact_Body");
  }
}
